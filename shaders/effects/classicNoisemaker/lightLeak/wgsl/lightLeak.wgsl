...
fn sample_base_color(uv : vec2<f32>, width : i32, height : i32) -> vec3<f32> {
    ...
    return textureLoad(inputTex, vec2<i32>(px, py), 0).xyz;
}
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let alpha : f32 = clamp01(params.alpha_timeSpeed.x);
    let time_value : f32 = params.alpha_timeSpeed.y;
    let speed_value : f32 = params.alpha_timeSpeed.z;
