...
float value_map(vec4 texel, uint channelCount) {
    if (channelCount == 1u) {
...
void main() {
    ...
    ivec2 atlas_dims_i = textureSize(glyphTexture, 0);
    ...
    float value_component = value_map(sample_texel, channelCount);
    ...
    vec4 glyph_texel = texture(glyphTexture, (vec2(glyph_coords) + vec2(0.5)) / vec2(textureSize(glyphTexture, 0)));
    float glyph_value = clamp01(glyph_texel.x);

    vec4 result = vec4(0.0);
    for (uint channel = 0u; channel < channelCount; channel = channel + 1u) {
