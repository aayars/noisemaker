...
fn sanitized_channelCount(raw : f32) -> u32 {
...
fn value_map(texel : vec4<f32>, channelCount : u32) -> f32 {
    if (channelCount == 1u) {
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let src_texel : vec4<f32> = textureLoad(inputTex, src_coords, 0);

    let atlas_dims : vec2<u32> = textureDimensions(glyphTexture);
    ...
    let sample_texel : vec4<f32> = textureLoad(inputTex, sample_coords, 0);

    let value_component : f32 = value_map(sample_texel, channelCount);
    ...
    let glyph_texel : vec4<f32> = textureLoad(glyphTexture, glyph_coords, 0);
