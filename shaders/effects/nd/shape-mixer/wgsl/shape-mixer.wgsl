...
fn posterize(d_in: f32, lev_in: f32) -> f32 {
    var lev = lev_in;
...
fn posterize2(d: f32, lev_in: f32) -> f32 {
    if (lev_in == 0.0) { return d; }
    let lev = lev_in + 0.1;
...
fn blendFloat(color1: f32, color2: f32, mode: i32, factor_in: f32) -> f32 {
    let factor = 1.0 - factor_in;
...
fn blendVec3(color1: vec3f, color2: vec3f, mode: i32, factor_in: f32) -> vec3f {
    let factor = 1.0 - factor_in;
    if (mode == 0) { return color1 + color2 * factor; }
