...
vec3 pal(float t) {
    float cursor = t * repeatPalette + rotatePalette * 0.01;
    vec3 color = paletteOffset + paletteAmp * cos(6.28318 * (paletteFreq * cursor + palettePhase));
...
vec3 applyPalette(float state) {
    float intensity = clamp(state, 0.0, 1.0);
    if (colorMode == 4) {
        float cursor = intensity;
        if (cyclePalette == -1) {
            cursor += time;
        ...
        vec3 base = pal(cursor);
        return mix(vec3(0.0), base, intensity);
    }
