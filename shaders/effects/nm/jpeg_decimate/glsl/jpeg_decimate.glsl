...
void main() {
    ...
    vec4 texel = texelFetch(input_texture, coords_i, 0);
    vec3 state_rgb = texel.xyz;
    float alpha = texel.w;

    float time_value = sanitized_time(time_speed.x);
    float speed_value = sanitized_speed(time_speed.y);
    ...
    for (uint iteration = 0u; iteration < INTERNAL_ITERATIONS; iteration = iteration + 1u) {
        ...
        vec3 sample_rgb = texelFetch(input_texture, sample_coords, 0).xyz;
