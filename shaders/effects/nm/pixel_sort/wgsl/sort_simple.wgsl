...
fn main(input : VertexOutput) -> @location(0) vec4<f32> {
    ...
    let size : vec2<i32> = vec2<i32>(textureDimensions(input_texture));
    let x : i32 = coord.x;
    let y : i32 = coord.y;
    ...
    for (var i : i32 = 0; i < width; i = i + 1) {
        let texel : vec4<f32> = textureLoad(input_texture, vec2<i32>(i, y), 0);
    ...
    for (var i : i32 = 0; i < width; i = i + 1) {
        ...
        for (var j : i32 = 0; j < width; j = j + 1) {
            ...
            let other : vec4<f32> = textureLoad(input_texture, vec2<i32>(j, y), 0);
