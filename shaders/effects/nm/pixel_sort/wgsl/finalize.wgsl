...
@group(0) @binding(0) var input_texture : texture_2d<f32>;  // sorted
...
fn main(input : VertexOutput) -> @location(0) vec4<f32> {
    let texSize : vec2<f32> = vec2<f32>(textureDimensions(input_texture));
    ...
    let sampledSorted : vec4<f32> = textureSample(input_texture, input_sampler, uvCoord);
