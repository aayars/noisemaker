...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let texel : vec4<f32> = textureLoad(inputTex, coords_i, 0);
    var state_rgb : vec3<f32> = texel.xyz;
    let alpha : f32 = texel.w;

    let time_value : f32 = sanitized_time(params.timeSpeed.x);
    let speed_value : f32 = sanitized_speed(params.timeSpeed.y);
    ...
    for (var iteration : u32 = 0u; iteration < INTERNAL_ITERATIONS; iteration = iteration + 1u) {
        ...
        let sample_rgb : vec3<f32> = textureLoad(inputTex, sample_coords, 0).xyz;
