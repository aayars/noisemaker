...
uniform sampler2D stats_texture;
...
void main() {
    ivec2 texSize = textureSize(input_texture, 0);
    ...
    vec4 texel = texelFetch(input_texture, pixel, 0);
    float referenceValue = value_map_component(texel);

    vec2 minMax = texelFetch(stats_texture, ivec2(0, 0), 0).xy;
    ...
    vec4 sampled = texelFetch(input_texture, ivec2(sampleX, sampleY), 0);
    fragColor = sampled;
}