...
vec3 sample_base_color(vec2 uv, int width, int height) {
    ...
    return texelFetch(input_texture, ivec2(px, py), 0).xyz;
}
...
void main() {
    ...
    float alpha = clamp01(alpha_time_speed.x);
    float time_value = alpha_time_speed.y;
    float speed_value = alpha_time_speed.z;
    ...
    vec4 base_sample = texelFetch(input_texture, ivec2(coords), 0);
