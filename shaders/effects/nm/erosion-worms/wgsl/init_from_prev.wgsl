// Flow Field GD - Pass 1: Initialize from previous frame
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let intensity_fade : f32 = clamp(params.controls1.x, 0.0, 1.0);
    let prev_color : vec4<f32> = textureLoad(prev_texture, vec2<i32>(i32(gid.x), i32(gid.y)), 0);
    let faded : vec4<f32> = prev_color * intensity_fade;
