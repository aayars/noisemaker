// Flow Field GD - Pass 2: Agent Movement
// Each thread processes one agent in parallel.
// Agent state: [x, y, x_dir, y_dir, r, g, b, inertia, age] (9 floats per agent)

const TAU : f32 = 6.283185307179586;

struct ErosionWormsParams {
    size : vec4<f32>, // (width, height, channels, unused)
    controls0 : vec4<f32>, // (density, stride, quantize, padding)
    controls1 : vec4<f32>, // (intensity, inverse, xy_blend, time)
    controls2 : vec4<f32>, // (worm_lifetime, inputIntensity, padding, padding)
};
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let worm_lifetime : f32 = params.controls2.x;
    ...
    let time : f32 = params.controls1.w; // Normalized 0..1
    
    // Normalize lifetime (0-60 range) to 0..1 for cycle duration
    let normalized_lifetime : f32 = worm_lifetime / 60.0;
    
    // Normalize agent index to 0..1 for staggered offsets
    let normalized_index : f32 = f32(agent_id) / f32(agent_count);
    
    // Agent's phase offset in the cycle
    let agent_phase : f32 = fract(normalized_index);
    let needs_initial_color : bool = age < 0.0;
    
    // Check if we're at the respawn point in this agent's cycle
    // Respawn occurs when (time + offset) wraps past the lifetime duration
    let time_in_cycle : f32 = fract(time + agent_phase);
    let prev_time_in_cycle : f32 = fract(time - (1.0 / 60.0) + agent_phase); // Assuming 60fps
    ...
    if (needs_initial_color) {
        let init_xi : i32 = wrap_int(i32(floor(x)), i32(width));
        let init_yi : i32 = wrap_int(i32(floor(y)), i32(height));
    ...
    if (respawn_check) {
        ...
        let seed : u32 = agent_id + u32(time * 1000.0);
        let pos : vec2<f32> = hash2(seed);
        x = pos.x * f32(width);
        y = pos.y * f32(height);

        // Sample spawn color from the current input texture
        let spawn_xi : i32 = wrap_int(i32(floor(x)), i32(width));
        let spawn_yi : i32 = wrap_int(i32(floor(y)), i32(height));
        ...
        // Randomize direction so respawned agents immediately move in a new heading
    ...
    // Single iteration per frame (like worms module)
    ...
    output_buffer[base + 3u] = clamp(output_buffer[base + 3u] + deposit_alpha * intensity, 0.0, 1.0);
