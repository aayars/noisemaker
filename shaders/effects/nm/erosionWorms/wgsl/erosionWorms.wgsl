...
fn luminance_at(x : i32, y : i32, width : u32, height : u32, channelCount : u32) -> f32 {
    let texel : vec4<f32> = fetch_texel(x, y, width, height);
    if (channelCount <= 1u) { return texel.x; }
    if (channelCount == 2u) { return texel.x; }
    let rgb : vec3<f32> = vec3<f32>(texel.x, texel.y, texel.z);
    return oklab_l(rgb);
}

fn blurred_luminance_at(x : i32, y : i32, width : u32, height : u32, channelCount : u32) -> f32 {
    var total : f32 = 0.0;
    ...
    for (var offset_y : i32 = -2; offset_y <= 2; offset_y = offset_y + 1) {
        for (var offset_x : i32 = -2; offset_x <= 2; offset_x = offset_x + 1) {
            let sample : f32 = luminance_at(x + offset_x, y + offset_y, width, height, channelCount);
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    if (gid.x < width && gid.y < height) {
        ...
        output_buffer[base_index + 0u] = faded.x;
        output_buffer[base_index + 1u] = faded.y;
        output_buffer[base_index + 2u] = faded.z;
        output_buffer[base_index + 3u] = faded.w;
    }
    ...
    if (gid.x == 0u && gid.y == 0u && gid.z == 0u) {
        ...
        if (floats_len >= 9u) {
            ...
            let trail_intensity : f32 = clamp(params.controls1.x, 0.0, 1.0);
            let inverse_flag : bool = params.controls1.y > 0.5;
            let time_value : f32 = params.controls1.w;
            let wormLifetime : f32 = max(params.controls2.x, 0.0);

            for (var agent_id : u32 = 0u; agent_id < agent_count; agent_id = agent_id + 1u) {
                ...
                let normalized_lifetime : f32 = wormLifetime / 60.0;
                ...
                let respawn_check : bool = wormLifetime > 0.0
                ...
                if (needs_initial_color || respawn_check) {
                    ...
                    let xi_seed : i32 = wrap_int(i32(floor(x)), i32(width));
                    let yi_seed : i32 = wrap_int(i32(floor(y)), i32(height));
                    let sample_color : vec4<f32> = textureLoad(inputTex, vec2<i32>(xi_seed, yi_seed), 0);
                    cr = sample_color.x;
                    cg = sample_color.y;
                    cb = sample_color.z;
                    age = 0.0;
                ...
                let c00 : f32 = blurred_luminance_at(xi, yi, width, height, channelCount);
                let c10 : f32 = blurred_luminance_at(x1i, yi, width, height, channelCount);
                let c01 : f32 = blurred_luminance_at(xi, y1i, width, height, channelCount);
                let c11 : f32 = blurred_luminance_at(x1i, y1i, width, height, channelCount);
                ...
                let pixel_idx : u32 = u32(yi2) * width + u32(xi2);
                let base_index : u32 = pixel_idx * 4u;
                ...
                output_buffer[base_index + 0u] = clamp(output_buffer[base_index + 0u] + deposit_rgb.x * trail_intensity, 0.0, 1.0);
                output_buffer[base_index + 1u] = clamp(output_buffer[base_index + 1u] + deposit_rgb.y * trail_intensity, 0.0, 1.0);
                output_buffer[base_index + 2u] = clamp(output_buffer[base_index + 2u] + deposit_rgb.z * trail_intensity, 0.0, 1.0);
                output_buffer[base_index + 3u] = clamp(output_buffer[base_index + 3u] + trail_intensity, 0.0, 1.0);
    ...
    if (gid.x < width && gid.y < height) {
        ...
        let input_sample : vec4<f32> = textureLoad(inputTex, vec2<i32>(i32(gid.x), i32(gid.y)), 0);
