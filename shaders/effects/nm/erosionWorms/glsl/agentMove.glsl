...
uniform sampler2D state1Tex;      // Agent position/direction
uniform sampler2D state2Tex;      // Agent color/inertia
uniform sampler2D state3Tex;      // Agent age
uniform sampler2D inputTex;   // Source image for gradient sampling
...
vec4 fetch_input(int x, int y, int width, int height) {
    ...
    return texelFetch(inputTex, ivec2(wx, wy), 0);
}
