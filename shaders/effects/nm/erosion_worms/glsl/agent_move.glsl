...
void main() {
    ...
    if (needs_initial_color || respawn_check) {
        ...
        vec4 sample_color = textureLoad(input_texture, vec2(xi, yi), 0);
        cr = sample_color.x;
        cg = sample_color.y;
        cb = sample_color.z;
        age = 0.0;
    ...
    uint pixel_index = uint(yi2) * width + uint(xi2);
