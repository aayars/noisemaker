...
float stripePattern(vec2 pixelPos, float angle, float period, float thickness, float t) {
    float c = cos(angle);
    float s = sin(angle);
...
void main() {
    ...
    vec4 base = texture(input_texture, uv);
    ...
    float stripesA = stripePattern(pixelPos, mix(-0.45, 0.35, h1), mix(60.0, 110.0, h2), 0.18, timeOffset * 0.25);
    float stripesB = stripePattern(pixelPos, mix(-1.1, 1.1, h2), mix(40.0, 80.0, h3), 0.12, timeOffset * 0.15);
    float stripesC = stripePattern(pixelPos, mix(-0.2, 0.2, h3), mix(90.0, 160.0, h1), 0.09, timeOffset * 0.35);
