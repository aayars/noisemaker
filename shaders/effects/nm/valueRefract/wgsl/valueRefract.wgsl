...
struct ValueRefractParams {
    ...
    displacement_timeSpeed_distrib : vec4<f32>, // (displacement, time, speed, distrib)
};
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let channelCount : u32 = max(as_u32(params.size_freq.z), 1u);
    let freq_param : f32 = params.size_freq.w;

    let displacement : f32 = params.displacement_timeSpeed_distrib.x;
    let time : f32 = params.displacement_timeSpeed_distrib.y;
    let speed : f32 = params.displacement_timeSpeed_distrib.z;
    let distribution_id : i32 = i32(round(params.displacement_timeSpeed_distrib.w));
    ...
    let base_index : u32 = (gid.y * width_u + gid.x) * channelCount;
    var channel : u32 = 0u;
    loop {
