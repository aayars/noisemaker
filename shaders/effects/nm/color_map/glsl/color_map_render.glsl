...
uniform sampler2D clut_texture;
uniform sampler2D statsTex; // (min, max, 0, 0) at (0,0)
...
void main() {
    ...
    vec4 texel = texelFetch(input_texture, ivec2(coord), 0);
    ...
    vec4 clut_sample = texelFetch(clut_texture, ivec2(sample_x, sample_y), 0);
