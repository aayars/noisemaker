...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let channelCount : u32 = sanitized_channelCount(params.channels);

    let coords : vec2<i32> = vec2<i32>(i32(gid.x), i32(gid.y));
    let src : vec4<f32> = textureLoad(inputTex, coords, 0);
    ...
    for (var c : u32 = channelCount; c < CHANNEL_COUNT; c = c + 1u) {
