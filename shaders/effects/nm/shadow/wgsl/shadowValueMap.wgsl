// Shadow value map pass - converts to luminance
// Matches shadowValueMap.glsl (GPGPU fragment shader)
...
fn srgbToLinearVec3(value: vec3<f32>) -> vec3<f32> {
...
fn oklabLComponent(rgb: vec3<f32>) -> f32 {
    let linear = srgbToLinearVec3(clamp(rgb, vec3<f32>(0.0), vec3<f32>(1.0)));
...
fn main(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let texel = textureSampleLevel(inputTex, inputSampler, uv, 0.0);
