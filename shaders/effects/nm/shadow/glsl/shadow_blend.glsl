...
uniform sampler2D sharpen_texture;
uniform float alpha;
...
void main() {
    ...
    vec4 baseColor = texture(input_texture, uv);
    float shadeNorm = texture(sobel_texture, uv).r;
    float sharpenNorm = texture(sharpen_texture, uv).r;
