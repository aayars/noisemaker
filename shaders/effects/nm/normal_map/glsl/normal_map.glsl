...
uint sanitize_channel_count(float raw_value) {
...
float compute_reference_value(ivec2 coords, uint channel_count) {
...
void main() {
    ...
    uint channel_count = sanitize_channel_count(size.z);
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float value = compute_reference_value(sample_coord, channel_count);
    ...
    vec4 texel = texelFetch(input_texture, ivec2(global_id.xy), 0);
