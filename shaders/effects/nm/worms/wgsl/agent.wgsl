// Worms agent pass - GPGPU agent simulation with MRT output
...
fn main(@builtin(position) position : vec4<f32>) -> Outputs {
    ...
    var worms_x = state1.x;
    var worms_y = state1.y;
    ...
    if (initialized < 0.5) {
        ...
        let xi = wrap_int(i32(worms_x), width);
        let yi = wrap_int(i32(worms_y), height);
    ...
    // Move agent
    var newX = worms_x + sin(finalAngle) * actualStride;
    var newY = worms_y + cos(finalAngle) * actualStride;
