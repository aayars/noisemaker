...
void main() {
    ...
    vec4 base_sample = textureLoad(input_texture, vec2(int(global_id.x), int(global_id.y)), 0);
    ...
    textureStore(output_texture, vec2(int(global_id.x), int(global_id.y)), vec4(combined_rgb, final_alpha));
}