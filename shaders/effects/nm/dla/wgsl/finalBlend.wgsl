...
fn palette(t: f32) -> vec3<f32> {
    return mix(vec3<f32>(0.05, 0.0, 0.08), vec3<f32>(1.0, 0.25, 0.9), clamp(t, 0.0, 1.0));
}
...
fn main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var clusterColor = cluster.rgb;
    
    if (dot(clusterColor, clusterColor) < 1e-4) {
        clusterColor = palette(intensity) * intensity;
    }
    
    let emission = clusterColor * (0.35 + intensity * 0.8);
    let combined = mix(inputColor.rgb, clamp(inputColor.rgb + emission, vec3<f32>(0.0), vec3<f32>(1.0)), clamp(alpha, 0.0, 1.0));
