...
@group(0) @binding(2) var<uniform> u_scale: f32;

/* Sums all channels, wrapping the magnitude to preserve variation. */
@fragment
fn main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
  let st = position.xy / vec2<f32>(textureDimensions(u_tex0, 0));
  let inputColor = textureSample(u_tex0, u_sampler, st);
  let summed = dot(inputColor, vec4<f32>(1.0));
  let scaled = summed * u_scale;
