/*
 * Main raymarching shader for nu/noise3d
...
vec3 shade(vec3 p, vec3 rd) {
    ...
    if (colorMode == 0) {
        ...
        // RGB mode - use stored RGB noise values from the volume
        vec4 volColor = sampleVolume(p);
        baseColor = volColor.rgb;
    }
...
vec3 shadeVoxel(vec3 p, vec3 rd, vec3 n, ivec3 voxel) {
    ...
    if (colorMode == 0) {
        ...
        // RGB mode - use stored RGB noise values from the voxel
