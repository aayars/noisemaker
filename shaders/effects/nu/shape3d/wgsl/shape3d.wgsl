/*
 * Main raymarching shader for nu/shape3d (WGSL)
...
// Shape params (loopA/B params) are used by precompute pass only.
...
fn calcNormal(p: vec3<f32>) -> vec3<f32> {
    ...
    // Central differences on the raw field
...
fn shadeVoxel(p: vec3<f32>, rd: vec3<f32>, n: vec3<f32>) -> vec3<f32> {
    let lightDir = normalize(vec3<f32>(1.0, 1.0, -1.0));
    ...
    if (colorMode == 0) {
        ...
        let faceShade = abs(n.x) * 0.9 + abs(n.y) * 1.0 + abs(n.z) * 0.85;
        baseColor = abs(n) * 0.5 + vec3<f32>(0.5 * faceShade);
    }
...
fn main(@builtin(position) position: vec4<f32>) -> FragmentOutput {
    ...
    if (filtering == 1) {
        ...
        if (hit.dist > 0.0) {
            ...
            color = mix(vec3<f32>(0.02), vec3<f32>(0.1), uv.y + 0.5);
        }
    } else {
        // Smooth isosurface mode
