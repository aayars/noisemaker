...
void main() {
    ...
    // Get offset values for both loops
    float offset1 = offset3D(p, lf1, loopAOffset, amp1);
    float offset2 = offset3D(p, lf2, loopBOffset, amp2);
    
    // Combine offsets with time (using z coordinate as a time-like dimension for 3D variation)
    float t1 = p.z;  // Use z as base time-like coordinate
    float t2 = p.z;
    ...
    // Apply periodic function
    float a = periodicFunction(t1);
    float b = periodicFunction(t2);
    
    // Combine with amplitudes - use product for more interesting 3D shapes
    // This creates fuller volume coverage than abs((a+b)-1.0)
    float combined = a * amp1 + b * amp2;
    
    // Normalize to [0, 1] range based on total amplitude
    float totalAmp = amp1 + amp2;
    float d = (totalAmp > 0.0) ? combined / totalAmp : 0.5;
