/*
 * Main raymarching shader for nu/shape3d
...
// Check if a voxel is solid (for voxel mode)
...
// DDA voxel traversal for voxel mode
struct VoxelHit {
    float dist;
    vec3 normal;
    ivec3 voxel;
...
vec3 shade(vec3 p, vec3 rd) {
    ...
    if (colorMode == 0) {
        ...
        // RGB mode - color based on normal direction
        baseColor = n * 0.5 + 0.5;
    }
...
vec3 shadeVoxel(vec3 p, vec3 rd, vec3 n) {
    vec3 lightDir = normalize(vec3(1.0, 1.0, -1.0));
    ...
    if (colorMode == 0) {
        ...
        float faceShade = abs(n.x) * 0.9 + abs(n.y) * 1.0 + abs(n.z) * 0.85;
        baseColor = abs(n) * 0.5 + vec3(0.5 * faceShade);
    }
...
void main() {
    ...
    if (filtering == 1) {
        ...
        if (hit.dist > 0.0) {
            ...
            color = shadeVoxel(p, rd, hit.normal);
            normal = hit.normal;
