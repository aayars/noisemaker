/*
 * Main raymarching shader for nu/ca3d (GLSL)
...
// For CA, we use the alive state (R channel)
...
vec3 shade(vec3 p, vec3 rd) {
    ...
    vec3 baseColor;
    if (colorMode == 0) {
        // Mono mode - white/gray
        baseColor = vec3(0.85);
    } else {
        // Age mode - color based on cell age (G channel)
        vec4 cached = sampleVolume(p);
        float age = cached.g;
        // Young cells are cyan, old cells are magenta
        baseColor = mix(vec3(0.2, 0.8, 1.0), vec3(1.0, 0.3, 0.8), age);
    }
...
vec3 shadeVoxel(vec3 p, vec3 rd, vec3 n, ivec3 voxel) {
    ...
    vec3 baseColor;
    if (colorMode == 0) {
        // Mono mode - subtle face shading
        float faceShade = abs(n.x) * 0.9 + abs(n.y) * 1.0 + abs(n.z) * 0.85;
        baseColor = vec3(0.8 * faceShade);
    } else {
