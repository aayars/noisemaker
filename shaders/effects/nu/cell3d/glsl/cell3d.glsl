/*
 * Main raymarching shader for nu/cell3d
...
// Volume texture dimensions from uniform
...
// Sample the precomputed volume cache with trilinear interpolation
// Returns vec4(normalizedDistance, colorR, colorG, colorB)
vec4 sampleVolume(vec3 worldPos) {
    int volSize = volumeSize;
    float volSizeF = float(volSize);
    
    // worldPos is in [-1, 1] space (bounding box)
    // Convert to [0, 1] for texture lookup
    ...
    vec4 c000 = texelFetch(volumeCache, ivec2(i0.x, i0.y + i0.z * volSize), 0);
    vec4 c100 = texelFetch(volumeCache, ivec2(i1.x, i0.y + i0.z * volSize), 0);
    vec4 c010 = texelFetch(volumeCache, ivec2(i0.x, i1.y + i0.z * volSize), 0);
    vec4 c110 = texelFetch(volumeCache, ivec2(i1.x, i1.y + i0.z * volSize), 0);
    vec4 c001 = texelFetch(volumeCache, ivec2(i0.x, i0.y + i1.z * volSize), 0);
    vec4 c101 = texelFetch(volumeCache, ivec2(i1.x, i0.y + i1.z * volSize), 0);
    vec4 c011 = texelFetch(volumeCache, ivec2(i0.x, i1.y + i1.z * volSize), 0);
    vec4 c111 = texelFetch(volumeCache, ivec2(i1.x, i1.y + i1.z * volSize), 0);
...
// In SDF mode: sdf = (noiseVal - threshold) means negative = inside
// So solid when noiseVal < threshold
...
// Uses fixed steps to detect threshold crossings, then bisection to refine
IsoHit isosurfaceTrace(vec3 ro, vec3 rd) {
    ...
    // Step size based on volume resolution - ensure we don't skip voxels
...
// Shading for smooth isosurface with cell-based coloring
vec3 shade(vec3 p, vec3 rd) {
    ...
    if (colorMode == 0) {
        ...
        // Use cached cell color (stored in GBA channels)
        vec4 cached = sampleVolume(p);
        baseColor = cached.gba;
    }
...
vec3 shadeVoxel(vec3 p, vec3 rd, vec3 n, ivec3 voxel) {
    ...
    vec3 baseColor;
    if (colorMode == 0) {
        // Mono mode - grayscale with subtle variation per face
        float faceShade = abs(n.x) * 0.9 + abs(n.y) * 1.0 + abs(n.z) * 0.85;
        baseColor = vec3(0.7 * faceShade);
    } else {
        // RGB mode - use cached cell color
        vec4 cached = sampleVoxel(voxel);
        baseColor = cached.gba;
    }
