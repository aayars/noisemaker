/*
 * Main raymarching shader for nu/rd3d (GLSL)
...
// For RD, we use chemical B concentration as the visible field
float getField(vec3 p) {
    float val = sampleVolume(p).g;  // Chemical B is in green channel
...
bool isVoxelSolid(ivec3 voxel) {
    float val = sampleVoxel(voxel).g;  // Chemical B
...
VoxelHit voxelTrace(vec3 ro, vec3 rd) {
    ...
    int volSize = volumeSize;
    float voxelSize = 2.0 / float(volSize);
...
vec3 shade(vec3 p, vec3 rd) {
    ...
    vec3 baseColor;
    if (colorMode == 0) {
        // Mono mode - grayscale based on B concentration
        baseColor = vec3(0.75);
    } else {
        // Gradient mode - color based on B concentration
        float b = sampleVolume(p).g;
        // Blue to orange gradient based on concentration
        baseColor = mix(vec3(0.2, 0.4, 0.8), vec3(1.0, 0.6, 0.2), b);
    }
