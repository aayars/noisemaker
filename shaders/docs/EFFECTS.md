...
└── example.dsl         # Example usage snippet
```
...
**Documentation**:
The `help.md` and `example.dsl` files are optional but recommended for library effects. They provide context and copy-pasteable examples for the editor UI.
...
2. **Fallback Table**: Apply backend-specific mapping:
   ...
   ```
3. **Precision Downgrade**: If no mapping exists, select highest precision supported format with same channel count.
4. **Fail**: If no compatible format, emit `ERR_FORMAT_UNSUPPORTED`.
